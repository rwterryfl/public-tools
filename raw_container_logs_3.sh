...
LOG_COUNT=$(oc exec -n openshift-logging $COLLECTOR_POD -- sh -c "cd /var/log/pods/$LOG_DIR/$CONTAINER_NAME && cat $(ls | grep -v '**.gz$')" | \
...
END_TIME=$(oc exec -n openshift-logging $COLLECTOR_POD -- sh -c "cd /var/log/pods/$LOG_DIR/$CONTAINER_NAME && ls -t | grep -v '**.gz$' | head -n 1 | xargs tail -n 1 | awk '{print $1} | xargs")
BEGIN_TIME=$(oc exec -n openshift-logging $COLLECTOR_POD -- sh -c "cd /var/log/pods/$LOG_DIR/$CONTAINER_NAME && ls -t | grep -v '**.gz$' | tail -n 1 | xargs head -n 1 | awk '{print $1} | xargs")
